...
interface ISafeTransfer {
    ...
    enum TransferStatus {
        PENDING,    /// Transfer is waiting to be claimed
        CLAIMED,    /// Transfer has been successfully claimed
        CANCELLED,  /// Transfer has been cancelled by sender
        EXPIRED,    /// Transfer has expired and can no longer be claimed
        NOT_FOUND   /// Transfer ID does not exist
    }

    /// @notice Structure representing a transfer
    /// @dev Contains all necessary information for a transfer lifecycle
    struct Transfer {
        address sender;         /// Address that initiated the transfer
        address recipient;      /// Address that can claim the transfer
        address tokenAddress;   /// Token contract address (0x0 for native ETH)
        uint256 amount;         /// Amount of tokens being transferred
        uint256 timestamp;      /// Block timestamp when transfer was created
        uint256 expiryTime;     /// Block timestamp when transfer expires
        bytes32 claimCode;      /// Optional claim code hash for additional security
        bool claimed;           /// Whether the transfer has been claimed
        bool cancelled;         /// Whether the transfer has been cancelled
    }
